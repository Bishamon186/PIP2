...
<%
    ...
    if (request.getAttribute("Result")!=null)
    {
        ...
        {
            out.write("Точка не попадает в область. <br>");
        }
    }
    else
    {
...
function Point(x1,y1,r1)//рисуем яркую красную точечку
{
    ...
    if (picture)
    {
        ...
        picture.moveTo(x1*40 + x, y1 - 40*y);
        picture.arc(x1*40 + x, y1 - 40*y, 1, 0, 2* Math.PI, false);
...
<%
    ...
    Vector yResult = new Vector();
    Vector rResult = new Vector();
    Vector hit = new Vector();
    Integer size = (Integer) session.getAttribute("Size");

    xResult = (Vector) session.getAttribute("x");
    yResult =(Vector) session.getAttribute("y");
    rResult =(Vector)  session.getAttribute("r");
    hit =(Vector)  session.getAttribute("hit");
    ...
    for (int i = 0; i < size; i++)
    {
        out.write("<tr>");
        out.write("<td>");
        out.print(xResult.get(i));
        out.write("</td>");
        out.write("<td>");
        out.print(yResult.get(i));
        out.write("</td>");
        out.write("<td>");
        out.print(rResult.get(i));
        out.write("</td>");
        out.write("<td>");
        if ((Boolean)hit.get(i))
            out.print("Попадает");
